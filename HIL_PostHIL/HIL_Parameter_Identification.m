close all, clear all
%% load data file
data_path = 'C:\Users\amirs\OneDrive - UC San Diego\college\research\Dr Tong ESS\Nissan cycle testing\raw data\HIL\Char6';
testStruct = dir(fullfile(data_path, '*.csv'));
tests = {testStruct.name};

%% initialize arrays for battery parameters
R0_all = zeros(length(tests),1); Rp_all = zeros(length(tests),1);
Cp_all = zeros(length(tests),1); 
RMSE_all = zeros(length(tests),1); MAE_all = zeros(length(tests),1);
...
function [Step, Cap, data] = getCellCap(fname)
    ...
    fid = fopen(fname,'r');
    Original = textscan(fid, '%f %s %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f', 'HeaderLines', 1,'Delimiter',',');
    ...
    for i = 1:length(t0)
        ...
        if i > 1
            X0 = str2double(split(t0(i-1),":"));
        end
        X1 = str2double(split(t0(i),":"));
        if i > 1 && X0(1)-X1(1) > 1
            tbase =tbase+X0(1)*60+X0(2);
        end
        t1(i) = tbase + X1(1)*60 + X1(2);
    end
    t1 = t1'./3600;
    ...
    StartVs = data(Step(14),4:end);
    EndVs = data(Step(19),4:end); %after pause
    ...
    CapDchAh = 25*(data(Step(19),2)-data(Step(18),2)); %battery DCH capacity from time at constant current
